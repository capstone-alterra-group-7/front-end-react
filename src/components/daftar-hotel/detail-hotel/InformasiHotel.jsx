    ...
    <div className="bg-[#EBEDF1] text-[#262627]">
      <div className="h-[26rem] bg-white px-8 py-6 flex">
        ...
        <div className="w-full">
          <div className="flex justify-between mb-2">
            <h1 className="text-[#262627] font-bold text-[2rem]">
              {data?.name}
            </h1>
          ...
          <h1 className="text-[#0080FF] font-semibold text-xl mb-2">
            {data?.id}
          </h1>
          ...
          <h1 className="text-[2.5rem] font-bold font-sans text-[#0080FF]">
            {rupiah(data?.price)}{" "}
            <span className="text-xl text-[#96989C]">/ kamar / malam</span>
          </h1>
        </div>
      </div>

      <div
        className={`mt-2 bg-white ${isHidden.fasilitas ? "h-[19rem]" : ""} p-8`}
      >
        ...
        <div className="pt-5 grid grid-cols-8 gap-y-3 ps-10">
          {data?.hotel_facilities.length < 1
            ...
            : data?.hotel_facilities
                .slice(
                  0,
                  isHidden.fasilitas ? 8 : data?.hotel_facilities.length
                )
                .map((facility, i) => {
                  return (
      ...
      <div className="mt-2 bg-white p-8">
        <div className="border-2 border-[#E1E4EA] rounded-lg pt-2 mb-8">
          ...
          {isHidden.desc ? null : (
            <div className="p-4 duration-500">{data?.description}</div>
          )}
        </div>
        ...
        <div className="border-2 border-[#E1E4EA] rounded-lg pt-2">
          ...
          >
            ...
            <img
              ...
              alt=""
              className={`h-5 w-4 duration-300 ${
                isHidden.kebijakan ? "rotate-180" : ""
              }`}
            />
          </div>
          {isHidden.kebijakan ? null : (
            <SectionKebijakan data={data?.hotel_policy} />
          )}
        </div>
