...
const ModalImageKamar = ({ setModal }) => {
  ...
  const imgUrl = [assets.imageHotel, assets.imageKamar, assets.imageKamar2, assets.imageKamar, assets.imageKamar2, assets.imageKamar, assets.imageKamar2, assets.imageKamar, assets.imageKamar2];
  ...
  return (
    <div className="fixed z-100 duration-500 top-0 right-0 left-0 bottom-0 flex flex-col bg-gray-900/75 h-screen">
      ...
      <div className="flex flex-col gap-5 h-full items-center">
        <DetailKamarCarousel imgUrl={imgUrl} indexImg={indexImg} setIndexImg={setIndexImg} />

        <div className={`flex flex-wrap ${imgUrl.length <= 4 ? "justify-center" : ""} w-4/5 gap-5 overflow-y-scroll 2xl:h-40 xl:h-36 h-28 xl:mt-4 scrollBar px-2`}>
          {imgUrl.map((url, idx) => (
            <img
              src={url}
              alt=""
              className={`2xl:w-40 xl:w-36 w-28 rounded-3xl cursor-pointer duration-100 ${indexImg === idx ? "border-4 border-[#0080FF]" : null}`}
