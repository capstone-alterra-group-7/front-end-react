...
const Ulasan = ({ data }) => {
  ...
  const { data: dataRating, isLoading } = useSWR(
    baseUrl(`/public/hotel/${data?.hotel_id}/rating?page=${changePage}&limit=20&filter=${urutkan}`),
    fetcherGet
  );
  ...
  const countPercentageRating = (i) => {
    ...
    return (dataRating?.data[`rating_${i}`] * 100) / dataRating?.data?.total_rating;
  };
  ...
  return (
    <div className="bg-white text-[#262627] flex py-4">
      <div className="w-1/3 h-full flex flex-col items-center border-r border-slate-200 ">
        ...
        <h1 className="text-[#96989C]">{dataRating?.data?.total_rating} Rating</h1>
            ...
            return (
              <div className="flex items-center mb-2" key={i}>
                ...
                <div className="w-full mr-4 bg-[#EBEDF1] rounded-full h-2.5 dark:bg-gray-700">
                  <div className="bg-[#0080FF] h-2.5 rounded-full" style={{ width: `${countPercentageRating(i + 1)}%` }}></div>
                </div>
                <h1 className="text-[#96989C] font-medium">{dataRating?.data[`rating_${i + 1}`]}</h1>
              </div>
            );
          })}
        </div>
      ...
      {dataRating?.data?.ratings === null ? (
        <div className="w-full flex flex-col items-center relative mb-10">
          ...
          <h1 className="font-bold text-2xl absolute bottom-2">Data rating tidak tersedia</h1>
        </div>
      ) : (
        <div className="px-8 pt-2 w-full">
          ...
          <div className="w-full flex justify-end">
            <div className="relative">
              ...
              <div onClick={() => setShowUrutan((prev) => !prev)} className="relative">
                ...
                <img
                  ...
                  className={`absolute top-[20px] right-[22px] duration-300 ${showUrutan ? "rotate-180" : ""}`}
                  alt="urutkan"
                />
              </div>
              ...
              {showUrutan ? (
                <div className="absolute mt-2 bg-white w-[256px] h-[105px] p-5 shadow-[0_3px_10px_rgb(0,0,0,0.2)] rounded-2xl space-y-4">
                  <div className="flex gap-3 items-center">
                    ...
                    <label className="text-[#262627] font-[400] text-[16px]">Oldest Rating</label>
                  </div>
                  ...
                  <div className="flex gap-3 items-center">
                    ...
                    <label className="text-[#262627] font-[400] text-[16px]">Latest Rating</label>
                  </div>
                </div>
              ) : null}
            </div>
                  ...
                  {Array(total)
                    ...
                    ?.map((_, index) => (
                      <div
                        ...
                        className={`${
                          changePage === index + 1 ? "text-[#0066CC]" : "text-gray-400"
