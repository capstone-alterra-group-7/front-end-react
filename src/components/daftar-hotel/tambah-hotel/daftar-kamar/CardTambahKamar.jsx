    ...
    <div className="p-8 bg-white rounded-[2rem] mb-7">
      ...
      <div className="p-7 pb-2 grid grid-cols-7">
        <div className="h-[21rem] mr-8 col-span-2">
          <img
            src={URL.createObjectURL(data?.hotel_room_image[0].imageFile)}
            alt=""
            ...
            <div className="mt-4 grid grid-cols-3 gap-2">
              {data?.hotel_room_image?.slice(1, 4).map((x, idx) => (
                <div className="relative">
                  <img
                    src={URL.createObjectURL(x?.imageFile)}
                    alt=""
                    className={`h-16 object-cover rounded-2xl`}
                    key={idx}
                  />
                  <div
                    className={`h-16 w-14 absolute top-0 opacity-80 text-white flex justify-center items-center rounded-2xl ${
                      idx === 2 ? "bg-gray-700" : ""
                    } `}
                  >
        ...
        <div className="col-span-5">
          ...
          <button className="p-3 border border-[#D2D7E0] w-full text-[#4B4C4E]" onClick={() => setShowDetail(true)}>
            Lihat Detail Kamar
          </button>
        </div>
      </div>
      ...
      <div className="mt-5 grid grid-cols-2 w-1/5 gap-x-5 gap-y-3">
        {data?.hotel_room_facility?.map((data, idx) => {
          return (
            <div className="flex" key={idx}>
              <img src={chooseIconFacility(data?.name)} alt="" className="mr-4" />
              <h1>{data?.name}</h1>
            </div>
          );
        })}
      ...
      {editRoom && <TambahKamar setAddingRoom={setEditRoom} name={"edit"} />}
    </div>
  );
};
